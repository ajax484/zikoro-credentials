import { FilterProps, TSelectedFilter } from "@/types/filter";
...
function DataFilter<T>({
  ...
  // Sort filters by order if provided
  ...
  const getDisplayValue = (filter: TSelectedFilter<T>) => {
    ...
    if (filter.value instanceof Date) {
      return format(filter.value, "dd MMM yyyy");
    }
    return filter.value;
  };
  ...
  return (
    <div className={className}>
      {type === "menu" ? (
        <Menubar className="flex justify-between px-1 border-0 !bg-transparent">
          {sortedFilters.map((filter) => (
            <MenubarMenu key={filter.accessor}>
              <MenubarTrigger
                className={cn(
                  "flex gap-0.5 items-center w-full min-w-fit justify-center px-0.5 !bg-transparent",
                  "border-l-[1px]" // Apply to all except first if needed
                )}
              >
                {filter.icon}
          ...
          <DropdownMenuContent>
            <Accordion type="single" collapsible>
              {sortedFilters.map((filter, index) => (
                <AccordionItem value={filter.label} key={index}>
                  <AccordionTrigger>
                    <div className="flex gap-0.5 items-center w-full min-w-fit justify-center px-0.5 !bg-transparent">
                      {filter.icon}
      ...
      {selectedFilters.length > 0 && (
        <div className="flex gap-2 flex-wrap px-2 justify-center mt-2">
              ...
              <button
                onClick={() =>
                  applyFilter(
                    filter.key,
                    filter.label,
                    filter.type === "multiple" || filter.type === "range"
                      ? []
                      : undefined
                  )
                }
