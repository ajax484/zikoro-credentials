...
export const GeneratorHeader: React.FC<GeneratorHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-200 px-3 sm:px-4 lg:px-8 py-2.5 sm:py-3.5 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          ...
          {STEPS.map((step, idx) => {
            ...
            return (
              <React.Fragment key={step.id}>
                {idx > 0 && (
                  <div
                    className={`h-0.5 w-5 lg:w-10 transition-colors ${
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 lg:gap-2 text-xs lg:text-sm font-medium transition-all ${
                    isCurrent
        ...
        <div className="flex md:hidden items-center">
          <span className="text-[11px] sm:text-xs font-semibold text-purple-700 bg-purple-50 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border border-purple-200">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          <Link
            href="/login"
            className="text-xs sm:text-sm font-medium text-gray-600 hover:text-gray-900 px-1.5 sm:px-2 py-1"
          >
            Log in
          </Link>
          <Link
            href="/signup"
            className="text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-custom-gradient-start to-custom-gradient-end px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:opacity-95 shadow-sm transition whitespace-nowrap"
          >
            Sign up
          </Link>
        </div>
